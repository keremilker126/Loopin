    ...
    private readonly List<string> _adminEmails = new()
{
   ""
    ...
    {
        return _adminEmails.Any(admin =>
            string.Equals(admin, (email ?? "").Trim(), StringComparison.OrdinalIgnoreCase));
    }
    ...
    public async Task<IActionResult> Register(RegisterDto dto)
    {
        ...
        dto.KullaniciAdi = (dto.KullaniciAdi ?? "").Trim();

        // 📌 ADIM 1: E-posta engellenmiş mi kontrol et
        // HomeController içindeki _blockedEmails listesine bakar
        if (HomeController._blockedEmails.Any(email =>
            string.Equals((email ?? "").Trim(), dto.Email, StringComparison.OrdinalIgnoreCase)))
        {
        ...
        var user = new Kullanici
        {
            ...
            Email = dto.Email,
            Sifre = _passwordService.HashPassword(dto.Sifre),
            EmailOnayli = false,
        ...
        var confirmationLink = $"https://loopin.fun/api/auth/verify?token={token}";
        var emailBody = $@"
...
", "text/html");
    ...
    public async Task<IActionResult> Login(LoginDto dto)
    {
        ...
        if (user == null || !_passwordService.VerifyPassword(user.Sifre, dto.Sifre))
            return BadRequest(new { message = "Geçersiz e-posta veya şifre!" });

        if (_passwordService.NeedsRehash(user.Sifre))
        {
            user.Sifre = _passwordService.HashPassword(dto.Sifre);
            await _context.SaveChangesAsync();
        }

        // 🔥 MAIL ONAYI KONTROLÜ (herkes için)
        if (!user.EmailOnayli)
        {
            return BadRequest(new
            {
                message = "Hesabınız henüz aktive edilmemiş.",
    ...
    <div style='font-family:Arial,sans-serif; background:#0f0f1a; padding:30px; color:white;'>
        ...
        return Ok(new
        {
            message = "Admin doğrulama kodu gönderildi",
            requires2FA = true
        });
    }
    ...
    public async Task<IActionResult> ChangePassword([FromBody] ChangePasswordDto dto)
    {
        ...
        if (!_passwordService.VerifyPassword(user.Sifre, dto.CurrentPassword))
            return BadRequest(new { message = "Mevcut şifreniz hatalı." });

        if (string.IsNullOrWhiteSpace(dto.NewPassword) || dto.NewPassword.Length < 6 || dto.NewPassword.Length > 64)
            return BadRequest(new { message = "Yeni sifre 6-64 karakter arasinda olmalidir." });

        // 3. Şifreyi güncelle
        user.Sifre = _passwordService.HashPassword(dto.NewPassword);
        await _context.SaveChangesAsync();
...
<div style='font-family:Arial,sans-serif; background:#0f0f1a; padding:30px; color:white;'>
    ...
    public async Task<IActionResult> ResetPasswordStep2([FromBody] ResetPasswordRequest request)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.NewPassword) || request.NewPassword.Length < 6 || request.NewPassword.Length > 64)
            return BadRequest(new { message = "Yeni sifre 6-64 karakter arasinda olmalidir." });

        // Şifreyi Güncelle
        user.Sifre = _passwordService.HashPassword(request.NewPassword);
    ...
    public async Task<IActionResult> DeleteUser(int id, [FromQuery] string adminEmail)
    {
        ...
        if (IsAdminEmail(user.Email))
            return BadRequest(new { message = "Admin kullanici silinemez" });
