using Loopin.Data;
using Loopin.Models;
using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore;

namespace Loopin.Controllers.Api;

[ApiController]
[Route("api/[controller]")]
public class AramaApiController : ControllerBase
{
    private readonly AppDbContext _context;
    private readonly ILogger<AramaApiController> _logger;

    public AramaApiController(AppDbContext context, ILogger<AramaApiController> logger)
    {
        _context = context;
        _logger = logger;
    }

    [HttpGet]
    public async Task<IActionResult> Search([FromQuery] string q)
    {
        if (string.IsNullOrWhiteSpace(q))
            return BadRequest(new { message = "Arama terimi bos olamaz." });

        q = q.Trim();
        if (q.Length > 100)
            return BadRequest(new { message = "Arama terimi cok uzun." });

        try
        {
            var searchTerm = q.ToLower();

            var videolar = await _context.Videolar
                .Include(v => v.Kullanici)
                .Where(v => v.Baslik.ToLower().Contains(searchTerm) ||
                            (v.Aciklama != null && v.Aciklama.ToLower().Contains(searchTerm)))
                .OrderByDescending(v => v.IzlenmeSayisi)
                .Select(v => new VideoDto
                {
                    Id = v.Id,
                    Baslik = v.Baslik,
                    VideoUrl = v.VideoUrl,
                    KapakResmiUrl = v.KapakResmiUrl,
                    Aciklama = v.Aciklama,
                    YuklenmeTarihi = v.YuklenmeTarihi,
                    IzlenmeSayisi = v.IzlenmeSayisi,
                    LikeSayisi = v.LikeSayisi,
                    KullaniciId = v.KullaniciId,
                    KullaniciAdi = v.Kullanici.KullaniciAdi ?? "",
                    AboneSayisi = v.AboneSayisi
                })
                .ToListAsync();

            var kullanicilar = await _context.Kullanicilar
                .Where(u => (u.KullaniciAdi ?? "").ToLower().Contains(searchTerm))
                .Select(u => new KullaniciDto
                {
                    Id = u.Id,
                    KullaniciAdi = u.KullaniciAdi,
                    Email = u.Email,
                    EmailOnayli = u.EmailOnayli
                })
                .ToListAsync();

            return Ok(new
            {
                query = q,
                videoCount = videolar.Count,
                userCount = kullanicilar.Count,
                videolar,
                kullanicilar
            });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Arama sirasinda hata olustu. Query: {Query}", q);
            return StatusCode(500, new
            {
                message = "Arama tamamlanamadi. Lutfen tekrar deneyin."
            });
        }
    }
}
